...
GitHub の Issue でチケット管理を行う。操作の細則は skill に分けてあり、ここでは要点だけを示す。

**GitHub の操作（→ github-workflow skill）**

- GitHub 操作は必ず `gh` コマンドで行い、ブラウザ前提の手順は踏まない
- Issue の要否は規模ではなく **挙動を変えるか** で決める。挙動を変える変更（バグ・ロジック・API 変更、実質的な doc/規約変更）は Issue に基づいて行い、Issue は「追加質問なしで着手できる粒度」で書いて対象ファイル・変更方針・受け入れ条件（テスト/検証方法）まで含める。挙動不変かつ些細な変更（typo・誤字・文言の微修正・コメント・整形）は Issue 不要
- **branch + PR は変更の性質によらず常に必須**。Issue を省ける場合でも `master` への直接 commit/push はせず、PR 本文に変更理由を記録として残す
- Issue 作成前に着手してよい（会話・探索で方針を詰めてから起票する流れを許可）。挙動変更で Issue が必要なら PR 作成前までに Issue を作り `Closes #<issue番号>` で紐付ける
- ブランチは用途別に命名する。`feature/<issue番号>-<説明>`、`bugfix/<issue番号>-<説明>`、`hotfix/<issue番号>-<説明>`、`docs/<issue番号>-<説明>` の形をとり、本流ブランチでは直接作業しない（Issue を省く些細な変更は番号なしブランチ可）
...
2. **作業契約（`AGENTS.md` と skill）** — 人間と AI が共通で守る約束。`AGENTS.md` に最小限の契約を置き、コミット規約・GitHub 操作・ドキュメント文体といった細則は skill（`git-commit` / `github-workflow` / `doc-writing`）に分けて必要なときに自動で読ませる。Issue やコミットの **品質** を担保する層。
...
- 作業前に `README.md` を読む（コミット・GitHub 操作・文書の細則は skill が必要なときに自動で読み込む）
- 変更は1ステップずつ入れ、直後に検証する
- コミットは「1コミット＝1論点」、メッセージは `type(scope): subject`
- 作成・更新したファイルの先頭に概要コメントを書く（この README にも付けてある）

細かい規約は skill に分けた。コミットの書き方は git-commit skill、
GitHub の操作手順は github-workflow skill、設計ドキュメントの文体は
doc-writing skill にある。github-workflow skill には
「GitHub 操作には必ず `gh` を使う」「ブラウザ前提の手順を書かない」というルールがあり、
これが今回のワークフロー全体の土台になっている。この README 自体も doc-writing skill の
...
する。これに合わせて github-workflow skill の記述も、存在しない `develop` 前提を消して
実際の `master` 運用へ直した。
